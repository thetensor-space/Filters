...
// Given a ascending sequence S and an element s, determine the location to insert s based on the order O.
// Assume that O(x,y) returns true if x le y.
__MySort := function( S, s, O )
  // Check if s in S.
  if s in S then
    return Index(S,s);
  end if;

  // Trivial case where |S|=1.
  if #S eq 1 then
    if O(S[1],s) then
      i := 2;
    else
  ...
  while i eq 0 do
    ...
    if O(S[n],s) then
      if O(s,S[n+1]) then
        ...
        if counter le Floor(Log(2,#S))-3 then
          next := Ceiling(AbsoluteValue( last[1] - last[2] )/2) + last[2];
        else
          next := last[2] + 1;
        end if;
      end if;
      ...
      if counter le Floor(Log(2,#S))-3 then
        next := last[2] - Ceiling(AbsoluteValue( last[1] - last[2] )/2);
      else
        next := last[2] - 1;
      end if;
    end if;
...
__InsertSorted := procedure( ~S, s, O )
  S := Insert( S, __MySort(S,s,O), s );
end procedure;
...
// Below are functions from Pete and James dealing with cyclic A-modules.
